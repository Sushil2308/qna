...
class ChatSessionManager(models.Model):
    ...
    sessionCreatedOn = models.DateTimeField(
        auto_now_add=True, help_text="When the session was created"
    )
    sessionLastAccessedOn = models.DateTimeField(
        auto_now=True, help_text="Session Last Accessed On"
    )


class SessionChatInfo(models.Model):
...
class SessionDocumentInfo(models.Model):
    ...
    canParticipate = models.BooleanField(
        default=True,
        help_text="Need to add this document while given answer to the user",
    )
    documentAddedOn = models.DateTimeField(
        auto_now_add=True, help_text="When the document was added to the session"
    )
    documentUpdatedOn = models.DateTimeField(
